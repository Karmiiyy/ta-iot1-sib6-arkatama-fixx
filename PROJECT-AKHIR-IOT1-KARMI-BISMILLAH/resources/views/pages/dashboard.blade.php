...
<div class="row">
    <div class="col-sm-12 col-md-7">
        <div class="card iq-mb-3">
            <div class="card-body">
                <h4 class="card-title">Monitoring Suhu</h4>
                <p class="card-text">Grafik berikut adalah monitoring sensor hujan 3 menit terakhir.</p>
...
<script>
    let chartGas, gaugeGas, chartHujan, gaugeHujan, chartSuhu, gaugeSuhu;
    ...
    async function requestchartSuhu() {
        // load data
        const result = await fetch("{{ route('api.sensors.dht11_suhu.index') }}");
        ...
        if (result.ok) {
            ...
            // refresh data setiap x detik
            setTimeout(requestchartSuhu, updateInterval); //1000ms = 1 detik
        }
    }

    async function requesGaugeSuhu() {
        // load data
        const result = await fetch("{{ route('api.sensors.dht11_suhu.index') }}");
        ...
        if (result.ok) {
            ...
            if (gaugeSuhu && !gaugeSuhu.renderer.forExport) {
                const point = gaugeSuhu.series[0].points[0];
                point.update(Number(value));
            }

            // refresh data setiap x detik
            setTimeout(requesGaugeSuhu, updateInterval); //1000ms = 1 detik
        }
    }
    ...
    window.addEventListener('load', function() {
        ...
        gaugeSuhu = new Highcharts.Chart({
